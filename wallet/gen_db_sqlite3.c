...
struct db_query db_sqlite3_queries[] = {
    ...
    },
    {
         ...
         .query = "SELECT  prev_out_tx, prev_out_index, value, type, status, keyindex, channel_id, peer_id, commitment_point, confirmation_height, spend_height, scriptpubkey , reserved_til FROM outputs WHERE status= ? ",
         .placeholders = 1,
         .readonly = true,
    },
    {
    ...
    },
    {
         ...
         .query = "SELECT  prev_out_tx, prev_out_index, value, type, status, keyindex, channel_id, peer_id, commitment_point, confirmation_height, spend_height, scriptpubkey, reserved_til FROM outputs WHERE prev_out_tx = ? AND prev_out_index = ?",
         .placeholders = 2,
         .readonly = true,
    },
    {
    ...
    },
    {
         ...
         .query = "SELECT  prev_out_tx, prev_out_index, value, type, status, keyindex, channel_id, peer_id, commitment_point, confirmation_height, spend_height, scriptpubkey , reserved_til FROM outputs WHERE status = ? OR (status = ? AND reserved_til <= ?)ORDER BY RANDOM();",
         .placeholders = 3,
         .readonly = true,
    },
    {
    ...
    },
    {
         ...
         .query = "SELECT * FROM (SELECT  id, status, destination, msatoshi, payment_hash, timestamp, payment_preimage, path_secrets, route_nodes, route_channels, msatoshi_sent, description, bolt11, failonionreply, total_msat, partid FROM payments ORDER BY id DESC LIMIT ?)x ORDER BY id ASC;",
         .placeholders = 1,
         .readonly = true,
    },
    {
